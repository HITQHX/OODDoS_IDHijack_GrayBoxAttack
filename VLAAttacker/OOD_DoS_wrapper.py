...
# 导入我们写的灰盒攻击器
from white_patch.OOD_DoS import OOD_DoS_Attacker
...
def main(args):
    ...
    path = f"{pwd}/run/OOD_DoS/{args.dataset}/{exp_id}"
    ...
    attacker = OOD_DoS_Attacker(vla, processor, save_dir=path, optimizer="adamW")
    ...
    print("Out-of-Distribution Semantic Denial of Service Attack done!")
...
def arg_parser():
    ...
    parser.add_argument('--patch_size', type=int, default=50, help="Square patch dimension")
...
if __name__ == "__main__":
    args = arg_parser()
    print(f"Parameters: malicious text:{args.malicious_text}\n lr:{args.lr} \n device:{args.device} \ntags:{args.tags}")
    main(args)